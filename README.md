...
支持文件夹穿透和随机播放的图片幻灯片播放Python脚本。支持播放背景音乐。
...
两个脚本均为键盘控制。可以直接拖入文件夹到Python文件开始播放，也可以通过F1到F12和数字0到9来播放预先设置好的库，最多支持12*10=120个库。支持读取Windows下的快捷方式指向的文件夹、视频和图片。支持播放.url文件所指的网络图片和视频。支持以英文、拼音、拼音首字母搜索文件夹。
...
我的一位朋友存了很多套图，分门别类地摆放整齐，分了好几级文件夹，还标注上了好看不好看。但是要看的时候就找不到合适的看图软件，大部分看图软件都只能在一个文件夹里随机播放幻灯片，其他的要么支持文件夹穿透但不能随机播放，要么能随机播放但只能穿透一层文件夹。实在是让人十分不快，所以他让我做一个专门随机看套图的软件。
...
**重点来了**，当你随机到一张图你觉得不错，想继续浏览这类图时，可以按Q、W、E键。如果此时正在随机播放，那么按下Q会使程序在该文件夹下进行随机播放，按下W会使程序在该文件夹的上一级文件夹下进行随机播放，按下E则会使程序在该文件夹的上两级文件夹下进行随机播放。按下R返回原来的随机模式。这也是我那位朋友非常喜欢的操作。
...
单击操作是给手机设计的方式，在手机上安装RD Client远程控制该电脑后，触屏就相当于鼠标单击，可以使用这种方式简单地使用该程序。另外RD Client附带的键盘具有PC上的所有按键，所以用手机也是能完全控制该程序的。
...
比如你想搜索名为“孙尚香”的文件夹（随便举个古人名），可以按 Tab键，输入 ssx ，再按Tab 来进入该文件夹，或者按 Tab键，输入sunshang，再按Tab 也可。
...
2. 幻灯片切换没有特效。
3. 显示一张图片时，我希望能有缓慢的缩放效果。但缩放中心不知道该怎么找。
